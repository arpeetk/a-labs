...
- **Checkpoint/restore** (`internal/{blob,podruntime}`) — when a checkpoint mount is enabled, the trusted sidecar writes periodic full-workspace tar.gz snapshots and hydrate restores the newest snapshot into a replacement PVC. GCS-FUSE is the production-shaped backend (`--checkpoint-gcs-mount`); `--checkpoint-local-path` exists only for repeatable single-node kind/dev recovery tests. The harness never receives the checkpoint mount. Full interval snapshots are real; incremental bundles, retention, event-triggered/final snapshots, and non-Codex transcript continuity remain future work (§5.5).
...
Also pending: `mcp`, `usage`, `project config`, and `run attach`/`steer` are deliberately absent from the CLI rather than shipped as stubs; they remain M1–M2 roadmap (`fleet` is now real). Checkpoint follow-ups are incremental/git-aware archives, retention, `checkpoint_hint` and SIGTERM snapshots, and transcript continuity for Claude Code/OpenCode. Also still open: historical/aggregated logs (GCS) and multi-restart `--previous` for `run logs` (live-tail is built), managed Postgres provisioning (the store impl exists — see above), gRPC/Connect transport, isolated agent node pool, and pause/resume semantics distinct from failure recovery.
...
wren run resume <id>             # manual resume of a terminal Failed run
wren run rm <id>
...
> **Current status (WS-21 + recovery hardening):** automatic recovery is a layered implementation. A retryable pod failure or externally vanished recorded pod advances a monotonic `attemptGeneration`, consumes the bounded automatic-retry budget once, and creates a `mode=resume` pod. A surviving PVC is reattached without re-cloning. With a checkpoint bucket plus an operator-enabled mount, confirmed PVC loss creates a fresh PVC and hydrate restores the newest periodic snapshot; with no mount it fails deterministically as `WorkspaceLost` rather than silently starting empty. Pod and PVC deletion events from one node-loss incident are coalesced into one retry. `wren run resume` resets the automatic retry budget but does not reset attempt identity or return the harness to start mode. Codex persists a per-run session under the workspace and uses `codex exec resume --last`, live-validated across an externally deleted pod. Claude Code/OpenCode remain workspace-only recovery. Current checkpoints are full tar.gz archives on a fixed interval; no `checkpoint_hint`/SIGTERM flush, incremental bundle, retention policy, or `status.lastCheckpoint` projection yet.
...
2. **Workspace checkpoints — mounted object storage.** The trusted checkpointer sidecar currently snapshots the full workspace to `runs/<id>/checkpoints/*.tar.gz` on a fixed interval (default 120s). GCS-FUSE is the production-shaped mount; local hostPath is test-only. Git-aware incremental bundles and event/final flushes are the next optimization, not current behavior.
...
| Stuck / infinite loop | Not detected automatically yet | Manual stop today; budgets/watchdog/pause are roadmap | Last periodic checkpoint only when enabled |
...
- **M0 — Foundations.** Control-plane skeleton (auth, projects, runs), operator + `AgentRun` CRD, **Claude Code** harness, async task→PR, Regional PD workspace (the GCS checkpointer moved post-launch — WS-8), hardened `runc` pod on an isolated agent node pool, egress-proxy, GitHub App + repo-scoped tokens, `run create/get/logs`. *(Journey A + C end-to-end.)*
  - **Done:** operator (`AgentRun` reconcile + classified, bounded crash recovery; monotonic attempt identity; missing-pod detection; PVC reattach; checkpoint restore after PVC loss); control-plane Runs/Projects services + HTTP API, running **in-cluster** (`config/default` Deployments + Service); CLI `run create/list/get/logs/stop/rm/resume` and `project create/list/get`, plus the real fleet view; CR-status mirroring; the `wren-runtime` image + the **real Claude Code agent** + **codex** (secure-egress and persisted-session crash resume live-validated)/**opencode** (not yet live-key-validated) adapters; periodic full-workspace checkpoints and restore through an isolated mounted store (GCS-FUSE production shape, local kind test backend); the **egress-proxy** (credential injection + allowlist, runner holds no secret) with **bypass enforcement** (iptables uid-lockdown + per-run canary, verified on real GKE Standard) and a DNS-rebinding-closed CONNECT path; the **Postgres store** alongside the in-memory default; idempotent, retry-classified **GitHub PR/finalize**; and onboarding as product surface via `wren install`. **Verified e2e:** Journey A on kind and real GKE; GCS snapshot/positive restore/no-checkpoint failure on GKE; local destructive pod+PVC recovery with byte-for-byte restored content; and a real Codex run resumed after external pod deletion and opened a PR. **Remaining:** GitHub App tokens wired per-run (the minter is built), Ingress/OIDC, isolated node pool, managed Postgres provisioning, gRPC transport, pause/resume, incremental/final/event-triggered checkpoints, and session parity for non-Codex harnesses.

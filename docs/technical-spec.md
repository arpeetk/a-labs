...
> **Status:** Draft v0.5 (living) · **Owner:** Platform / Software Factory · **Last updated:** 2026-08-10
>
...
- **M0 — Foundations.** Control-plane skeleton (auth, projects, runs), operator + `AgentRun` CRD, **Claude Code** harness, async task→PR, Regional PD workspace (the GCS checkpointer moved post-launch — WS-8), hardened `runc` pod on an isolated agent node pool, egress-proxy, GitHub App + repo-scoped tokens, `run create/get/logs`. *(Journey A + C end-to-end.)*
  - **Done:** operator (`AgentRun` reconcile + classified, bounded crash recovery; monotonic attempt identity; missing-pod detection; PVC reattach; verified checkpoint restore after PVC loss; durable pause/resume); control-plane Runs/Projects services + HTTP API, running **in-cluster** (`config/default` Deployments + Service); CLI `run create/list/get/logs/stop/pause/rm/resume` and `project create/list/get`, plus the real fleet and desktop views; CR-status/checkpoint/condition mirroring; the `wren-runtime` image + the **real Claude Code agent** + **codex** (secure-egress and persisted-session crash resume live-validated)/**opencode** (not yet live-key-validated) adapters; atomic checksummed full-workspace checkpoints with retention and exact paused restore through an isolated mounted store (GCS-FUSE production shape, local kind test backend); the **egress-proxy** (credential injection + allowlist, runner holds no secret) with **bypass enforcement**; the **Postgres store**; idempotent, retry-classified **GitHub PR/finalize**; and onboarding via `wren install`. **Remaining:** GitHub App tokens wired per-run, Ingress/OIDC, isolated node pool, managed Postgres provisioning, gRPC transport, incremental/final/event-triggered checkpoints, and session parity for non-Codex harnesses.

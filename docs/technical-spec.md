...
> **Status:** Draft v0.5 (living) · **Owner:** Platform / Software Factory · **Last updated:** 2026-08-13
>
...
- **CLI** (`cmd/wren`) — `login`, `install`/`uninstall`, `project create/list/get`, `run create/list/get/logs/stop/rm`, `fleet` wired to the control-plane HTTP API (`run logs [-f] [--container]` streams pod logs — resolves the run's current pod by the `wren.dev/run` label and tails the `pods/log` subresource; `run list`/`fleet` render a table by default with `--project`/`--phase` filters and `--watch` for a live view, WS-20). **Zero placeholder commands** (WS-15): everything `--help` lists actually works; `mcp`/`usage`/`run attach`/`run steer`/`project config` were removed from the CLI surface rather than shipped as commands that error — they're roadmap (M1–M2), documented in SETUP.md, not stubs. (`fleet` was on that removed list too, until WS-20 made it real.)
- **Operator** (`cmd/wren-operator`, `internal/controller`) — `AgentRun` reconciler (hardened pod + workspace PVC + RunSpec ConfigMap, lifecycle, automatic crash recovery, monotonic attempt generations, bounded retry accounting, PVC reattach, and checkpoint restore after confirmed PVC loss). Externally deleted pods are treated as interrupted attempts rather than silently recreated in start mode; one pod+PVC loss incident consumes one retry even when Kubernetes reports the two deletions separately, and a speculative replacement pod is removed if the same incident becomes terminal `WorkspaceLost`. CRDs + RBAC + manager manifests live under `config/`. (The `AgentPool` skeleton mentioned in earlier drafts was removed as dead code — no runs ever took the warm-pool path; `AgentPool` stays a **target-only** M3 design, §5.4/§10.)
- **Control plane** (`cmd/wren-apiserver`, `internal/{apiserver,coreapi,store,launcher}`) — Runs + Projects services; atomically commits each run with a leased Kubernetes-launch intent and submission event, replays idempotently across replica death, mirrors CR status, and stores an ordered deduplicated event journal. Postgres migrations serialize across HA replicas and `/readyz` verifies the database. `config/production-gcp` provides the two-replica Cloud SQL Auth Proxy/PDB deployment shape; managed instance provisioning remains external.
- **Harness runtime** (`cmd/wren-runtime`, `internal/{harness,podruntime}`) — the multi-call in-pod binary implementing the §5.4 contract's batch subset (RunSpec in, event stream + exit-code contract out; interactive input lands in M2). Adapters: **claude-code** (real — drives the bundled `claude` CLI headless in the cloned workspace and parses its stream-json events into Wren events + token usage), **codex** (Responses HTTP/SSE through secure egress, live-canary validated; private `CODEX_HOME` persisted on the workspace and `codex exec resume --last` live-validated across pod loss), **opencode** (WS-12: built and unit-tested, not yet run against the live provider; see docs/harnesses.md), and **mock** (deterministic, keyless — always resolves to the runtime image regardless of any harness-image default, so it never depends on a project's `--harness-image` being set correctly). Images: `build/Dockerfile.{runtime,claude-code,codex,opencode}`, all built + pushed/loaded automatically by `wren install` (WS-14).
- **Checkpoint/restore** (`internal/{blob,podruntime}`) — when a checkpoint mount is enabled, the trusted sidecar writes periodic full-workspace archives and hydrate restores a verified snapshot into a replacement PVC. Archives are immutable, read-back SHA-256 verified, and made visible atomically by a versioned manifest written last; retention defaults to five manifest/archive pairs. Safe relative workspace symlinks are preserved, while absolute or escaping links to ephemeral tool installations are omitted so creation and secure extraction accept the same archive contract. A user pause quiesces the harness, forces this verified checkpoint, commits its identity to status, then removes the pod. GCS-FUSE is the production-shaped backend (`--checkpoint-gcs-mount`); `--checkpoint-local-path` exists only for repeatable single-node kind/dev recovery tests. The harness never receives the checkpoint mount. Incremental and event-triggered/final snapshots and non-Codex transcript continuity remain future work (§5.5).
- **Egress-proxy** (`internal/egress`) — the pod's controlled egress: enforces a domain allowlist and **injects the GitHub token + model key** for github.com / api.github.com / api.anthropic.com / api.openai.com (the `/openai/` Responses HTTP/SSE route is live-canary validated with Codex). The credentials live only on the proxy container; the runner holds no secret. CONNECT resolves the target host once and dials the resolved IP directly (WS-16), closing a DNS-rebinding TOCTOU window in the allowlist check.
- **GitHub PR / finalize** (`internal/{github,gitwork,finalize}`) — go-git clone/commit/push (distroless-friendly, no git binary) + open a PR with the rubric body. Hydrate clones and finalize pushes/opens the PR *through the egress-proxy*. Finalize is idempotent and retry-classified (WS-11): a resumed run reuses its existing branch instead of failing "already exists," and transient push/PR failures (network, 429/5xx) retry while permanent ones (401/403/422) fail fast. A clean worktree still counts as changed when HEAD contains harness-created commits descended from the hydrate-time `origin/<baseRef>`; unrelated or replacement history is rejected rather than pushed.
...
- **Store:** two impls behind `Store` plus the production `Durable` extension — **in-memory** for dev/tests and **Postgres** (pgx/v5, embedded forward-only migrations). Postgres transactions include the run, launch outbox item, and first event; workers claim with leases/`SKIP LOCKED`; event source IDs deduplicate replay. On boot the apiserver also reconciles CR projections. The Cloud SQL deployment overlay is built; managed instance/database/IAM provisioning remains a target (§5.2).
...
- **GitHub credentials:** a **PAT** (mounted into the egress-proxy) for M0; the target is per-run, repo-scoped **GitHub App installation tokens** minted by the control plane (§5.7).
...
- **Run events and result projection:** the harness mirrors JSONL to shared pod IPC. The gateway forwards it through the trusted egress proxy, which injects authentication and binds the run identity; `(run, source, source_id)` makes gateway restart replay harmless. The operator still scrapes terminal `pr_ready`/`token_usage` events into CR status as an independent safety net. The untrusted harness and gateway hold no control-plane token or service-account token.
...
3. ~~**`wren install --create-cluster`**~~ — **done**: a bounded, opt-in GKE Standard quickstart provisions a cluster through `gcloud`, while `wren uninstall --delete-cluster --confirm` provides its explicit destructive counterpart. Bring-your-own-cluster remains the recommended production flow. The disposable real-GKE verification runner independently refuses to adopt an existing cluster, falls back across capacity pools, and verifies deletion after both egress and checkpoint gates.

Also pending: `mcp`, `usage`, `project config`, and `run attach`/`steer` are deliberately absent from the CLI rather than shipped as stubs; they remain M1–M2 roadmap (`fleet` is now real). Checkpoint follow-ups are incremental/git-aware archives, `checkpoint_hint` and SIGTERM snapshots, and transcript continuity for Claude Code/OpenCode. Also still open: historical/aggregated logs (GCS) and multi-restart `--previous` for `run logs` (live-tail is built), managed Postgres provisioning (the store impl exists — see above), gRPC/Connect transport, and an isolated agent node pool.
...
> **Current reality:** resume recreates the pod, reattaches a surviving workspace PVC, and starts the harness with `RunSpec.mode=resume`. If checkpointing is configured and the PVC is confirmed lost, Wren provisions a new PVC and hydrate restores the newest periodic full-workspace snapshot; without a configured mount, the same loss fails deterministically as `WorkspaceLost`. Codex also resumes its private persisted session. Claude Code/OpenCode currently recover the workspace but begin a fresh conversation (§5.5).
...
| Harness adapters | `internal/harness` | claude-code (real), codex (live-canary validated), opencode (built, not live-validated), mock; the event protocol |
| Egress-proxy | `internal/egress` | allowlist + credential injection |
| GitHub / PR | `internal/{github,gitwork,finalize}` | GitHub API client, go-git ops, finalize→PR |
...
- **Pausing** — harness quiesced while a forced checkpoint is published and verified.
- **Paused** — verified checkpoint recorded and agent pod removed; no compute is running.
...
wren run pause <id>              # verified checkpoint, then stop compute
wren run resume <id>             # resume a Paused or terminal Failed run
wren run rm <id>
...
> **M0 status:** the Runs and Projects services are built (`internal/coreapi`) over a `Store` interface plus a durable Postgres outbox/event extension. Accepted runs survive API death before Kubernetes publication, leased replicas replay idempotently, and reconcile-on-boot repairs CR projections. The one-way gateway event uplink is built; bidirectional attach/steer remains later. The API is **HTTP/JSON** (`internal/apiserver`, `cmd/wren-apiserver`); gRPC/Connect and Sessions/MCP/Usage/Audit services remain later milestones.
...
spec:
  ...
  workspace:
    ...
    checkpoint: { intervalSeconds: 120, bucket: gs://wren-ckpt }  # active when the operator enables a checkpoint mount (§5.5)
  mcp:
...
status:
  ...
  podName: r-8f3a2c-0
  restartCount: 1
  lastCheckpoint: { uri: gs://wren-ckpt/r-8f3a2c/ck-000042, at: "…", commit: "wip-abc" }  # target status projection; snapshots exist but this pointer is not populated yet
  sessionId: sess-…
...
**Reconciliation (AgentRun):** *(every run currently takes the cold path — `AgentPool` claim/hand-off lands in M3; checkpoint restore is active only when a checkpoint mount is enabled)*
...
| **checkpointer** | With an enabled mount, performs a startup Put/Get/List self-check and writes full-workspace tar.gz snapshots on `intervalSeconds`; hydrate can restore the newest one after PVC loss. GCS-FUSE is opt-in via `--checkpoint-gcs-mount`; a local hostPath backend exists for single-node kind/dev tests. Without a mount it is a liveness sidecar. Event-triggered/SIGTERM snapshots, incremental bundles, retention, and continuous transcript mirroring are not built. The bucket credential and mount live here/hydrate, never on the harness. | Trusted |
...
- **Claude Code** — implemented as a headless Claude CLI adapter with stream-json event parsing; native session persistence/resume remains to be added.
- **Codex** — implemented as a Codex CLI adapter. Its private `CODEX_HOME` lives under the durable workspace and replacement pods use `codex exec resume --last`; if an older run has no stored session it degrades explicitly to workspace-only recovery.
...
> **Current status (WS-22):** automatic recovery is layered. A retryable pod failure or externally vanished recorded pod advances a monotonic `attemptGeneration`, consumes the bounded automatic-retry budget once, and creates a `mode=resume` pod. A surviving PVC is reattached without re-cloning. With a bucket plus an operator-enabled mount, confirmed PVC loss creates a fresh PVC and hydrate chooses the newest valid manifest, skipping corrupt newer periodic checkpoints; with no mount it fails deterministically as `WorkspaceLost`. User pause is a separate contract: `Running -> Pausing` quiesces PID 1, forces and verifies an atomic checkpoint, records `status.lastCheckpoint`, removes the pod, then reports `Paused`. Failure unquiesces the live harness and records an actionable condition. Resume pins an exact pause checkpoint if the PVC disappeared, advances attempt identity once, and does not consume the crash budget. Failed-run resume still resets that budget. Codex persists a per-run session under the workspace; Claude Code/OpenCode remain workspace-only recovery.
...
2. **Workspace checkpoints — mounted object storage.** The trusted checkpointer archives the full workspace to an immutable `checkpoints/objects/<id>.tar.gz`, reads it back and verifies size/SHA-256, then atomically publishes `checkpoints/<id>.json` as the commit marker. Safe relative symlinks remain links; absolute and path-escaping links are omitted rather than followed or published, which keeps ephemeral CLI installation links out of the durable workspace without weakening extraction. Restore ignores orphan archives and validates every manifest; periodic recovery may fall back over corruption, while an explicit pause restore never does. Five pairs are retained by default. GCS-FUSE is the production-shaped mount; local hostPath is test-only. Git-aware incremental bundles and event/final flushes are the next optimization.
3. **Session state.** Codex keeps its private CLI state under the durable workspace, so it survives PVC reattach and is included in full checkpoints. Continuous, independently stored transcript mirroring and equivalent native resume for the other harnesses remain targets.
...
- The harness starts in **resume mode**. Codex continues its persisted session; other current adapters inspect the recovered workspace in a fresh conversation.
...
|---|---|---|---|
| OOMKilled | Pod `lastState.terminated.reason=OOMKilled` | Recreate, reattach PVC, resume harness | Live PVC intact; last checkpoint ≤ configured interval old |
| Evicted (preemption/pressure) | Pod failure/disappearance | Reschedule, reattach or restore, resume | Surviving PVC or last periodic checkpoint |
| Node failure / zone loss | Recorded pod disappears / PVC becomes NotFound | New pod; restore from checkpoint when configured, otherwise fail `WorkspaceLost` | Checkpoint ≤ configured interval old |
| Harness crash (non-zero) | Runner exit code | Retryable → resume; fatal → Fail | Workspace intact; Codex session intact, other transcripts not guaranteed |
| Control-plane outage | Health checks | Runs keep executing; status reconciles on recovery | CR is source of truth |
| Stuck / infinite loop | Not detected automatically yet | User can request safe pause; automatic watchdog/budget trigger is roadmap | Forced verified pause checkpoint |
...
> **M0 status:** the **finalize** step is built (`internal/{github,gitwork,finalize}`): go-git clone/commit/push + open a PR with the rubric body. It preserves commits authored by a harness before finalization, distinguishes them from a genuine no-op against the hydrate-time remote-tracking base, and fails closed if the candidate history is not descended from that requested base. Real live PRs have been produced end-to-end against `arpeetk/a-labs` on kind and GKE, with the token injected at the **egress-proxy** (the runner holds nothing). Remaining vs. the target: (1) the token is a **PAT** in the proxy secret rather than a per-run App installation token; (2) the branch is `wren/<sanitized-user>/<run-id>` — the `-<slug>` suffix and rubric *validation* are not yet implemented; (3) the App *setup* flow (`wren install`) is pending.
...
1. Finalize creates or reuses `wren/<user>/<run-id>-<slug>` from `baseRef`. A harness may commit first; finalize preserves commits descended from the hydrate-time `origin/<baseRef>`, but never publishes unrelated history.
...
- **M0 — Foundations.** Control-plane skeleton (auth, projects, runs), operator + `AgentRun` CRD, **Claude Code** harness, async task→PR, Regional PD workspace (the GCS checkpointer moved post-launch — WS-8), hardened `runc` pod on an isolated agent node pool, egress-proxy, GitHub App + repo-scoped tokens, `run create/get/logs`. *(Journey A + C end-to-end.)*
  - **Done:** operator (`AgentRun` reconcile + classified, bounded crash recovery; monotonic attempt identity; missing-pod detection; PVC reattach; verified checkpoint restore after PVC loss; durable pause/resume); crash-safe control plane with atomic launch outbox, HA leases/migrations, immutable gateway event journal, and Postgres-aware readiness; HTTP API running **in-cluster** (`config/default`) plus the Cloud SQL HA shape (`config/production-gcp`); CLI `run create/list/get/logs/stop/pause/rm/resume` and `project create/list/get`, plus the real fleet and desktop views; CR-status/checkpoint/condition mirroring; the `wren-runtime` image + the **real Claude Code agent** + **codex** (secure-egress and persisted-session crash resume live-validated)/**opencode** (not yet live-key-validated) adapters; atomic checksummed full-workspace checkpoints with retention and exact paused restore through an isolated mounted store; the **egress-proxy** with bypass enforcement; idempotent, retry-classified **GitHub PR/finalize**; and onboarding via `wren install`. **Remaining:** GitHub App tokens wired per-run, Ingress/OIDC, isolated node pool, managed Cloud SQL lifecycle, gRPC transport, bidirectional steering, incremental/final/event-triggered checkpoints, and session parity for non-Codex harnesses.
...
2. **Checkpoint granularity vs cost** — full-workspace archives are real and default to a 120s interval. Large working trees need retention and tuning now; git-aware incrementals, sparse checkout, and interval backoff should reduce future storage/CPU cost.

...
- **Checkpoint/restore** (`internal/{blob,podruntime}`) — when a checkpoint mount is enabled, the trusted sidecar writes periodic full-workspace archives and hydrate restores a verified snapshot into a replacement PVC. Archives are immutable, read-back SHA-256 verified, and made visible atomically by a versioned manifest written last; retention defaults to five manifest/archive pairs. A user pause quiesces the harness, forces this verified checkpoint, commits its identity to status, then removes the pod. GCS-FUSE is the production-shaped backend (`--checkpoint-gcs-mount`); `--checkpoint-local-path` exists only for repeatable single-node kind/dev recovery tests. The harness never receives the checkpoint mount. Incremental and event-triggered/final snapshots and non-Codex transcript continuity remain future work (§5.5).

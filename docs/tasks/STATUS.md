...
| **21** | **Real checkpointing: periodic snapshots + restore-from-checkpoint** | [WS-21](WS-21-checkpoint-restore.md) | ready | `../wren-ws21` (via `/goal`) | builds directly on WS-18/19's GCS mount: real tar.gz workspace snapshots on an interval, restore-latest-checkpoint into a freshly-recreated PVC when the workspace is confirmed lost (extends the mount to `hydrate`, read-only, never `harness`); falls back to WS-16's existing loud `PhaseFailed`/`WorkspaceLost` when checkpointing isn't configured or no checkpoint exists — reuses `classifyTermination`'s existing init-container exit-code handling for that, no new failure-classification code. **Not self-merge-eligible** (changes reconciler retry semantics + trust-tier mount) — stops at PR-ready-for-review |

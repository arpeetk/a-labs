...
| **18** | **GCS mount for the checkpointer** | [WS-18](WS-18-gcs-mount.md) | ready | — | owner asked "do we support GCS mount" — answer was no (`internal/blob.Store` is an interface, zero impls). This connects it for real: GKE Cloud Storage FUSE CSI driver mounted into the checkpointer sidecar ONLY (never the harness — new trust-tier invariant, tested), a new Workload Identity binding (first use in this repo), a mount-backed `blob.Store` impl. Explicitly NOT the real periodic checkpointer (still deferred, WS-8) — just proving the socket works, live, on real GCP |

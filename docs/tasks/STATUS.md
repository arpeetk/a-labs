...
> **Priority call (2026-07-29, owner):** the mission is "any engineer can
> easily spin up coding agents in the cloud, offload tasks, track their
> progress, and orchestrate them." Measured against that, fleet visibility —
> not sandbox isolation (gVisor) or checkpointing — is the biggest gap: today
> there's no way to see more than one run at a time. WS-20 closes it. The
> sandbox-substrate question (keep Wren's own operator vs. build on
> `sigs.k8s.io/agent-sandbox`) is deliberately deferred, not decided —
> revisit when gVisor/checkpointing actually come up, not before; see the
> published comparison for the reasoning. WS-20 is also this project's first
> workstream run via `/goal` (autonomous build/test/validate loop, no
> per-PR human review gate) — see its brief's "Autonomous execution rules."

> **Priority call (2026-07-30, owner):** with fleet visibility done, next up
> is closing checkpointing's real gap — WS-18/19 proved the GCS mount works,
> but the checkpointer never actually took a snapshot and a lost workspace PVC
> always failed the run outright (WS-16), even when a checkpoint existed.
> WS-21 builds on that mount directly: real periodic tar.gz snapshots of the
> workspace, and restore-from-latest-checkpoint before the harness starts when
> the PVC is confirmed lost — falling back to WS-16's loud deterministic
> failure when no checkpoint exists. Unlike WS-20, this one changes the
> reconciler's failure/retry semantics and extends a trust-tier mount, so it
> is **not** self-merge-eligible — brief stops at PR-ready-for-review.

> **Priority call (2026-08-11, owner):** make Wren's recovery model a product
> contract rather than an implementation detail. WS-22 delivers user-controlled
> pause/resume backed by an atomic, verified checkpoint, exact restore,
> integrity/retention metadata, and visible CLI/desktop recovery state. The work
> is autonomous through implementation and exhaustive local/live validation,
> but merges only after the complete gate and CI evidence are green.
...
| **19** | **GCS FUSE mount under default egress enforcement** | [WS-19](WS-19-egress-gcsfuse-compat.md) | merged | #31 | closes WS-18's known gap: the mount self-check now passes under the default `--egress-enforcement=iptables` (proven live on real GKE 1.35, independently `gcloud storage cat`-verified). **Landed outcome B, not A** — the preferred proxy-routing was empirically ruled out: the CSI injection webhook *discards* env/args on a user-declared `gke-gcsfuse-sidecar` (HTTPS_PROXY stripped, verified), and gcsfuse exposes no proxy mount option, so the sidecar cannot be pointed at the egress-proxy. Instead: pin `storage.googleapis.com` to the restricted Google APIs VIP (a fixed /30) + `metadata.google.internal` to `169.254.169.254` via pod `hostAliases`, and add a narrow lockdown exemption scoped by BOTH the sidecar's uid (**65534**, verified — NOT root as WS-18's older CSI driver reported) AND those two CIDRs. Load-bearing check proven live: a process at the harness uid (65532) is BLOCKED from both destinations while the sidecar uid (65534) reaches them. Requires Private Google Access on the node subnet (documented prerequisite). Inert for every run without the mount; WS-1 canary + `make e2e` both modes unaffected |
| **20** | **Fleet visibility: `run list` + `wren fleet`** | [WS-20](WS-20-fleet-visibility.md) | merged | `../wren-ws20` (via `/goal`) | done — `run list`/`fleet` render a table by default (`--output json` for the old behavior); `--project` wired end-to-end for the first time (`store.RunFilter.Project` existed since before this workstream, nothing ever set it); `--phase` filters client-side; staleness fixed by reusing WS-3's `ReconcileFromCluster`/`launcher.ListRuns` bulk CR-list before every read, not N+1; `wren fleet` is real (removed as a WS-15 stub, re-added for real — flipped the test that asserted it was unknown); `--watch` polls/redraws/exits cleanly on Ctrl-C. **Live-verified on kind, not just unit-tested** — the freshness fix specifically: two runs across two projects, `--phase` showed one still `Provisioning` while the other had already reached `Succeeded` at the same instant, and `--watch`'s AGE column was observed incrementing across real poll cycles (33s→35s→37s→39s), proving genuine re-fetch each frame, not a cached snapshot. `make test vet`/lint/`make e2e` all green; kind cluster + port-forwards torn down clean. First workstream run via `/goal` (autonomous build/test/validate loop, no per-PR human review gate) — merged per the brief's own "Autonomous execution rules" once every Definition of Done item was genuinely observed true |
| **21** | **Real checkpointing: periodic snapshots + restore-from-checkpoint** | [WS-21](WS-21-checkpoint-restore.md) | merged | #33 | done — builds directly on WS-18/19's GCS mount: `blob.RunPrefix` (fixes the WS-18 self-check's unscoped-bucket-listing gap) + `blob.Archive`/`Unarchive` (stdlib tar+gzip, path-escape rejected); `RunCheckpointer` now really ticks on `WREN_CHECKPOINT_INTERVAL` and Puts `checkpoints/ck-<ts>.tar.gz`; `RunHydrate` restores the latest checkpoint when `RunSpec.RestoreRequired` (mandatory, not best-effort — no checkpoint is a plain non-retryable error, same `ExitError`→`classifyTermination`→`PhaseFailed` path WS-16 already built, zero new controller failure-classification code); `hydrate` gets the checkpoints mount read-only (never `harness`, extends `TestBuildAgentPod_GCSMount_HarnessNever*`); `ensurePVC` now branches on restore-eligibility (`CheckpointGCSMount` + a bucket) — ineligible runs keep the unchanged `errWorkspaceLost`→`PhaseFailed`, eligible runs delete the dead pod/bump `RestartCount`/set `WorkspaceRestorePending`→`errWorkspaceRestoring`→requeue, and the follow-up reconcile creates the fresh PVC with `RestoreRequired=true` threaded into the RunSpec; the condition clears once the new pod reaches `Running`. **Live-verified on real GKE** (`wren-e2e`, GKE 1.35.6, GCS FUSE CSI + Workload Identity, torn down after): periodic snapshots proven real (4-7 landed across attempts, cross-checked via independent `gcloud storage ls -l`/`cat`, content matches the archived source byte-for-byte); the full real `wren run create`→controller→hydrate pipeline proven for all three scenarios — positive restore (mid-run `kubectl delete pod+pvc` together — deleting only the PVC doesn't work against a live pod, `pvc-protection` finalizer blocks it until the referencing pod is gone too — controller recreated the PVC, hydrate restored the latest real checkpoint, **actual restored file content verified** via a debug pod mounting the same PVC, run reached `Succeeded`), negative (no checkpoint ever taken → deterministic `PhaseFailed`/`HarnessError`, not a retry loop), and regression (`--checkpoint-gcs-mount` off → unchanged pre-WS-21 `WorkspaceLost` failure, verified against the same project/bucket to isolate the operator flag as the only variable). Catching the mock harness's real `AgentRun` mid-`Running` (it completes in under a couple seconds) needed a tight watch-and-delete race — reliable enough in ~5 attempts to prove all three controller scenarios directly, no mock-harness code changes needed. **Interesting non-bug finding:** seeding a test checkpoint via a direct `gcloud storage cp` (bypassing any GCS FUSE mount) was invisible to a live mount's directory listing even 10+ seconds later — a GCS FUSE implicit-directory quirk specific to objects never written through a POSIX/FUSE path; real checkpoints (always written by `blob.MountStore.Put`'s real `MkdirAll`+`Create`, proven both by the checkpointer sidecar and by hydrate reading a different pod's mount) are unaffected — a live-verification-methodology artifact, not a product bug, worth remembering for the next workstream that hand-seeds GCS test fixtures. `make test vet`/`golangci-lint`/`make e2e` green in both egress-enforcement modes. **Follow-up, same PR:** the manual live verification above is now also scripted as `hack/e2e-gke-checkpoint.sh` (+ `make e2e-gke-checkpoint`), which surfaced and fixed two real bugs in the act of automating it — a bare `wait` in the workspace-break helper that hung on the long-lived apiserver port-forward (not just the two delete jobs it meant to wait on), and a checkpoint-seeding race (seeding after catching the run `Running` reliably loses to the mock harness's near-instant completion; now seeds concurrently with the race). Re-run clean end to end against a real cluster after both fixes: all four scenarios (periodic snapshots, positive restore, negative restore, regression) passed via the script, `make test vet`/lint/`make e2e` both modes re-confirmed green, infra torn down. **Merged per explicit owner instruction** (test end-to-end, add e2e coverage, merge if green) overriding the brief's default human-review-gate — see PR #33 for the full record |
| **22** | **Durable pause/resume + verified checkpoints** | [WS-22](WS-22-pause-resume.md) | merged | #42 | done — restart-safe Pausing/Paused lifecycle; forced atomic SHA-256-verified checkpoint manifests and exact restore; retention/corruption handling; CLI + native desktop recovery surfaces; Postgres projection; harness mount isolation preserved. Proven by unit/race/lint/coverage/assets/frontend/native desktop gates, kind pause chaos, both keyless egress modes, and a live Codex-backed pause/resume with workspace continuity. Initial CodeQL found three high-severity symlink-pivot paths in archive extraction; replaced path-based extraction with directory-scoped `os.Root`, added the adversarial regression, and re-ran CodeQL clean. All PR checks green before merge |
| **23** | **Crash-safe control plane + durable journal + operations UX** | [WS-23](WS-23-control-plane-durability.md) | done | `codex/control-plane-durability-ux` | atomic run/outbox/event commit; leased idempotent launch replay; HA migration lock; authenticated gateway journal; Cloud SQL production overlay; segmented/readable desktop UX. Recovery chaos, clean kind, native UI, race, and disposable real-GKE egress gates passed. |

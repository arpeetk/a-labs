...
| **17** | **`wren install --create-cluster`** | [WS-17](WS-17-create-cluster.md) | merged | #28 | done — live-proved end to end on real GCP (no pre-existing cluster → working control plane → mock run Succeeded → torn down); idempotent reuse + validation + preflight unit-tested. Live proof surfaced a real default-sizing bug (fixed same day, `1aa7ac5`): `e2-standard-2` doesn't leave enough allocatable CPU past GKE's own system pods for even one default 2-CPU run to schedule — bumped default to `e2-standard-4`. `wren uninstall --delete-cluster` noted as a natural follow-up, not built |

...
| **20** | **Fleet visibility: `run list` + `wren fleet`** | [WS-20](WS-20-fleet-visibility.md) | ready | `../wren-ws20` (via `/goal`) | biggest gap against the "track progress and orchestrate them" mission — today only one run at a time is visible. Real table output, `--project`/`--phase` filters (the store's `RunFilter.Project` already exists, just never wired end-to-end), a fix for the known store-staleness gap (reuse WS-3's CR-listing path, not N+1 reads), a real `wren fleet`, `--watch`. First workstream dispatched via `/goal` — autonomous, no per-PR human review gate; see the brief's "Autonomous execution rules" |
